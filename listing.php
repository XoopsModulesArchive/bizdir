...
// $Id: listing.php,v 1.3 2004/10/02 00:08:10 scurtescu Exp $
//  ------------------------------------------------------------------------ //
...
$lid = (int)$_GET['lid'];
$sql = sprintf('UPDATE %s SET hits = hits+1 WHERE lid = %u AND status > 0', $xoopsDB->prefix('bizdir_listing'), $lid);
$xoopsDB->queryF($sql);
$GLOBALS['xoopsOption']['template_main'] = 'bizdir_listing.html';
require XOOPS_ROOT_PATH . '/header.php';

$xoopsTpl->assign('module_name', $xoopsModule->name());

$result = $xoopsDB->query(
    'select l.lid, l.cid, l.title, l.Contact, l.address, l.address2, l.city, l.state, l.zip, l.country, l.phone, l.fax, l.email, l.url, l.logourl, l.status, l.date, l.hits, l.rating, l.votes, l.comments, t.description from ' . $xoopsDB->prefix('bizdir_listing') . ' l, ' . $xoopsDB->prefix(
        'bizdir_text'
...
$xoopsTpl->assign(
    ...
    [
        ...
        'mail_body' => rawurlencode(sprintf(_MD_INTLINKFOUND, $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/bizdir/listing.php?lid=' . $lid),
    ]
);
